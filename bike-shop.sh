  ...
  then
    ...
    then
      ...
      then
        ...
        RENTAL_ID=$($PSQL "SELECT rental_id FROM rentals INNER JOIN customers USING(customer_id) WHERE bike_id = '$BIKE_ID_TO_RETURN' AND phone = '$PHONE_NUMBER' AND date_returned IS NULL")
        # if input not rented
        if [[ -z $RENTAL_ID ]]
        then
          # send to main menu
